...
##        self.listOfWinFrame.append(seventhFrame)
        ...
        WindowLabel.place(x=150, y=30)
        WindowLabel.configure(background="green", font=("Times New Roman", 18, "bold"))
...
### Browse Button for Welcome Frame ###
    def browseWindow(self):
        FILEOPENOPTIONS = dict(defaultextension='*.*', filetypes=[('tif','*.tif'),('jpeg','*.jpeg'), ('jpg','*.jpg'), ('All Files','*.*')])
        self.fileName = filedialog.askopenfilename(**FILEOPENOPTIONS)
        image = PIL.Image.open(self.fileName)           ##Opening Image
